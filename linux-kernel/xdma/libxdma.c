		       ...
		       int *h2c_channel_max, int *c2h_channel_max)
{
	...
	if (rv)
       ...
       // regs_test(xdev,0x05050505);
        //regs_test(xdev,0x50505050);
        //regs_test(xdev,0x0a0a0a0a);
        //regs_test(xdev,0xa0a0a0a0);
...
#if 0
 void H2C_ddr_size(struct xdma_dev *xdev, u32 value)
 {
          u32 v;
          struct inspur_regs *reg =
                  (struct inspur_regs *)(xdev->bar[0]);

          write_register(value, &reg->h2c_ddr_size ,0x0c);
          v = read_register(&reg->h2c_ddr_size);
          printk("H2C_ddr_size ===========================reg->h2c_ddr_size = 0x%x", v);
 }
#endif

void regs_test(struct xdma_dev *xdev, u32 value)
 {
          u32 v ;
          struct regs_base *reg =
          		(struct regs_base *)(xdev->bar[0]);

	  struct regs_src_addr *reg_5 =
          		(struct regs_src_addr *)(xdev->bar[0] + 0x100);

	  struct regs_dst_addr *reg_6 =
          		(struct regs_dst_addr *)(xdev->bar[0] + 0x200);

	  struct regs_calc_start_reset *reg_8 =
          		(struct regs_calc_start_reset *)(xdev->bar[0] + 0x300);

	  struct regs_tq_dma_done *reg_9 =
          		(struct regs_tq_dma_done *)(xdev->bar[0] + 0x400);

	  struct regs_cacl_state *reg_10 =
          		(struct regs_cacl_state *)(xdev->bar[0] + 0x500);

	  struct regs_cacl_done *reg_11 =
          		(struct regs_cacl_done *)(xdev->bar[0] + 0x600);

	  struct regs_cacl_err *reg_12 =
          		(struct regs_cacl_err *)(xdev->bar[0] + 0x700);

	  struct regs_cacl_done_irq *reg_13 =
          		(struct regs_cacl_done_irq *)(xdev->bar[0] + 0x800);

	  struct regs_cacl_time *reg_14 =
          		(struct regs_cacl_time *)(xdev->bar[0] + 0x900);
		  
		  
          //1. RO
          v = read_register(&reg->major_version_l);
          printk(" ===========================reg->major_version_l = 0x%x", v);
	  //1.1. RO
          v = read_register(&reg->minor_version_l);
          printk(" ===========================reg->minor_version_l = 0x%x", v);
	  //2. 
          write_register(value, &reg->tupledesc_addr_l ,0x10);
          v = read_register(&reg->tupledesc_addr_l);
          printk(" ===========================reg->tupledesc_addr_l = 0x%x", v);
		  
	  write_register(value, &reg->tupledesc_addr_h ,0x14);
          v = read_register(&reg->tupledesc_addr_h);
          printk(" ===========================reg->tupledesc_addr_h = 0x%x", v);

	  write_register(value, &reg->tupledesc_len_l ,0x18);
          v = read_register(&reg->tupledesc_addr_l);
          printk(" ===========================reg->tupledesc_len_l = 0x%x", v);
		  
          //write_register(value, &reg->tupledesc_len_h ,0x1c);
          //v = read_register(&reg->tupledesc_len_h);
          //printk(" ===========================reg->tupledesc_len_h = 0x%x", v);
          //3. 
          write_register(value, &reg->qual_addr_l ,0x20);
          v = read_register(&reg->qual_addr_l);
          printk(" ===========================reg->qual_addr_l = 0x%x", v);
		  
	  write_register(value, &reg->qual_addr_h ,0x24);
          v = read_register(&reg->qual_addr_h);
          printk(" ===========================reg->qual_addr_h = 0x%x", v);

	  write_register(value, &reg->qual_len_l ,0x28);
          v = read_register(&reg->qual_len_l);
          printk(" ===========================reg->qual_len_l = 0x%x", v);

          //5. 
	  write_register(value, &reg_5->src_addr0_l ,0x00);
          v = read_register(&reg_5->src_addr0_l);
          printk(" ===========================reg_5->src_addr0_l = 0x%x", v);
		  
	  write_register(value, &reg_5->src_addr0_l ,0x04);
          v = read_register(&reg_5->src_addr0_l);
          printk(" ===========================reg_5->src_addr0_l = 0x%x", v);

	  write_register(value, &reg_5->src_len0_l ,0x08);
          v = read_register(&reg_5->src_len0_l);
          printk(" ===========================reg_5->src_len0_l = 0x%x", v);
		  
          //6.
          write_register(value, &reg_6->dst_addr0_l ,0x00);
          v = read_register(&reg_6->dst_addr0_l);
          printk(" ===========================reg_6->dst_addr0_l = 0x%x", v);
		  
	  write_register(value, &reg_6->dst_addr0_h ,0x04);
          v = read_register(&reg_6->dst_addr0_h);
          printk(" ===========================reg_6->dst_addr0_h = 0x%x", v);

		  write_register(value, &reg_6->dst_len0_l ,0x08);
          v = read_register(&reg_6->dst_len0_l);
          printk(" ===========================reg_6->dst_len0_l = 0x%x", v);

	  //8.
	  write_register(value, &reg_8->calc_start0_l ,0x00);
          v = read_register(&reg_8->calc_start0_l);
          printk(" ===========================reg_8->calc_start0_l = 0x%x", v);
		  
	  write_register(value, &reg_8->calc_reset0_l ,0x08);
          v = read_register(&reg_8->calc_reset0_l);
          printk(" ===========================reg_8->calc_reset0_l = 0x%x", v);


	  //9.
	  write_register(value, &reg_9->tupledesc_dma_done_l ,0x00);
          v = read_register(&reg_9->tupledesc_dma_done_l);
          printk(" ===========================reg_9->tupledesc_dma_done_l = 0x%x", v);
		  
	  write_register(value, &reg_9->qual_dma_done_l ,0x10);
          v = read_register(&reg_9->qual_dma_done_l);
          printk(" ===========================reg_9->qual_dma_done_l = 0x%x", v);


          //10.
	  write_register(value, &reg_10->cacl_state0_l ,0x00);
          v = read_register(&reg_10->cacl_state0_l);
          printk(" ===========================reg_10->cacl_state0_l = 0x%x", v);

	  //11.
	  write_register(value, &reg_11->cacl_done0_l ,0x00);
          v = read_register(&reg_11->cacl_done0_l);
          printk(" ===========================reg_11->cacl_done0_l = 0x%x", v);
	  
	  //12.
	  write_register(value, &reg_12->cacl_err0_l ,0x00);
          v = read_register(&reg_12->cacl_err0_l);
          printk(" ===========================reg_12->cacl_err0_l = 0x%x", v);

	  //13.
	  write_register(value, &reg_13->calc_done_irq0_l ,0x00);
          v = read_register(&reg_13->calc_done_irq0_l);
          printk(" ===========================reg_13->calc_done_irq0_l = 0x%x", v);

	  //14.
	  write_register(value, &reg_14->src_input_start_time0_l ,0x00);
          v = read_register(&reg_14->src_input_start_time0_l);
          printk(" ===========================reg_14->src_input_start_time0_l = 0x%x", v);

	  write_register(value, &reg_14->src_input_done_time0_l ,0x08);
          v = read_register(&reg_14->src_input_done_time0_l);
          printk(" ===========================reg_14->src_input_done_time0_l = 0x%x", v);

	  write_register(value, &reg_14->src_calc_done_time0_l ,0x10);
          v = read_register(&reg_14->src_calc_done_time0_l);
          printk(" ===========================reg_14->src_calc_done_time0_l = 0x%x", v);

		  
	  //7. 		  
	  write_register(value, &reg->reset_all_l ,0x40);
	  v = read_register(&reg->reset_all_l);
	  printk(" ===========================reg->reset_all_l = 0x%x", v);
